 ...
 * @param {HighlightedLocation} [props.startLocation] - Optional starting location to highlight on the map
...
export const GridMap: React.FC<PathSegmentsProps> = ({
  ...
  if (floorIndex < 0 || floorIndex >= allFloorData.length) {
    console.error(
      `Invalid floor number: ${currentFloor}. Available floors: 1-${allFloorData.length}`
    );
    return (
  ...
  const currentFloorData = allFloorData[floorIndex]!;
  ...
  const gridWithBathrooms = currentFloorData.bathrooms.reduce(
    (grid: CellType[][], bathroom: Bathroom) => {
      let updatedGrid = grid;
  ...
  const gridWithElevators = currentFloorData.elevators.reduce(
    (grid: CellType[][], elevator: Elevator) => {
      let updatedGrid = grid;
  ...
  const gridWithUtility = currentFloorData.utilityRooms.reduce(
    (grid: CellType[][], room: UtilityRoom) => {
      let updatedGrid = grid;
  ...
  const gridWithFire = currentFloorData.fireEquipment.reduce(
    (grid: CellType[][], equipment: FireEquipment) => {
